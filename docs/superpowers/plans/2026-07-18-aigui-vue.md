# @aigui/vue Implementation Plan (sub-project 4: Vue adapter)

> **For agentic workers:** REQUIRED SUB-SKILL: superpowers:subagent-driven-development. Steps use `- [ ]`.

**Goal:** A Vue 3 adapter mirroring `@aigui/react`: a `useAIRenderer()` composable and an `<AIRenderer>` component that stream LLM output into live Vue vnodes, render cards as Vue components, and route `onCardAction`.
...
**Tech Stack:** Vue 3 (peer), TypeScript, tsdown, Vitest + @vue/test-utils + jsdom. Depends on `@aigui/core`.
...
- [ ] **Step 7** `pnpm install && pnpm exec vitest run smoke` (vue smoke passes) + `pnpm --filter @aigui/vue build` (emits dist).
- [ ] **Step 8** Commit `chore: @aigui/vue package scaffold`
