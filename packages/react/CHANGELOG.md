...
- 54c3297: Render elements that carry no props, instead of silently degrading the whole tree to plain text.
...
- 54ae930: Honour HTML boolean attributes in the React adapter, so `defaultOpen` opens.

  `evidence({ defaultOpen: true })` rendered a collapsed block. The plugin emits
  `open: ""` — attribute-_presence_ semantics, which is what the DOM and the vanilla
  adapter want. React reads props as _values_, and an empty string is falsy, so it
