...
- "graph3d"：实体和带类型的边组成的知识图谱，用 GPU 跑力导向布局。nodes（必填，1–2000 个，[{"id", "name"（最多 80 字符）, "type"（最多 32 字符，同名的 type 在整面墙上颜色一致）, "value"（大小，默认按度数）}]）；edges（必填，最多 5000 条，[{"from", "to", "type"（最多 32 字符）}]，两端必须是上面的 node id）；types（最多 32 项，把某个 type 指定成某个十六进制颜色）；focus（某个 node id，会高亮并且永远带标签）；rotate（默认 true，图在眼前自己散开；false 直接给最终布局）。度数最高的 20 个带标签，其余悬停可见，右下角有类型图例
...
"graph3d" — entities and typed edges as a knowledge graph, laid out on the GPU. nodes (required, 1 to 2000, as {id, name at most 80 characters, type at most 32 characters — one type is one colour everywhere on the wall, value — how big, defaulting to its degree}); edges (required, at most 5000, as {from, to, type at most 32 characters}, both ends being node ids); types (at most 32 entries, pinning a type to a hex colour); focus (a node id, highlighted and always labelled); rotate (default true: the layout settles in front of the reader; false hands over the settled graph). The twenty busiest entities carry a label, the rest are one hover away, and the types are keyed in the corner.
